...
const DashboardView = ({ showNotification }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch usuarios
        const usuariosRes = await axios.get('http://localhost:3000/ https://financiera-backend.vercel.app/api/usuarios', { headers });
        if (usuariosRes.data.success) {
          const usuarios = usuariosRes.data.data;
          const vendedores = usuarios.filter(u => u.idRol === 2).length;

          // Fetch vehiculos
          const vehiculosRes = await axios.get('http://localhost:3000/ https://financiera-backend.vercel.app/api/catalogo', { headers });
