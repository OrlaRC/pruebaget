...
const VentasView = () => {
  ...
  const fetchMarcas = async () => {
    try {
      ...
      const res = await fetch('http://localhost:3000/ https://financiera-backend.vercel.app/api/marcas', {
  ...
  const fetchPublicaciones = async () => {
    try {
      ...
      const res = await fetch('http://localhost:3000/ https://financiera-backend.vercel.app/api/catalogo/admin', {
        headers: { Authorization: `Bearer ${accessToken}` }
      });
      const { success, data } = await res.json();
      if (success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:3000/ https://financiera-backend.vercel.app/api/ventas', {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Estás seguro de eliminar esta venta?')) {
      try {
        ...
        const res = await fetch(`http://localhost:3000/ https://financiera-backend.vercel.app/api/ventas/${id}`, {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${accessToken}` }
        });
